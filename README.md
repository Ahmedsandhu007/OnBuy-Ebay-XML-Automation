...
| `EAN` | The real barcode extracted from eBay's listing data, when it has one - blank otherwise. Informational only; also written to the Sheet's `EAN` column if it exists. **Not** what's sent to OnBuy - see below. |
...
| `Product URL` (optional, added 2026-07-06) | The canonical live product page OnBuy's own `/v2/queues` history reports for a successful submission - confirmed to sometimes differ from whatever the Add Listing page's own search links to. Only populated by `backfill_onbuy_status.py` (this field isn't in `create_product`/`update_listing`'s own responses). **You must add a `Product URL` column yourself** to both the Sheet header row and the Supabase table before this does anything - both `generate_xml.py` and `backfill_onbuy_status.py` check for the column's presence first and silently skip writing it if it's missing, so nothing breaks if you haven't added it yet, but nothing gets filled in either. |
